...
public class OrderQueryService {
    ...
    public ShippingInfoDTO getOrderShippingInfo(Long orderId) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Order not found"));
        return buildShippingInfo(order);
    }

    @Transactional(readOnly = true)
    public ShippingInfoDTO getOrderShippingInfoForUser(Long orderId, Long userId) {
        Order order = getOrderForUser(orderId, userId);
        return buildShippingInfo(order);
    }
    ...
    private ShippingInfoDTO buildShippingInfo(Order order) {
